 ...
 */
public abstract class InstallFit {
    /**
     * 设置标题样式、内容等
     * @param book 文件对象
     * @param sheet sheet页
     ...
     * @author qixin
     */
    public static void fitTitle(HSSFWorkbook book, HSSFSheet sheet, String fileName, int witch, String currentUser) {
     ...
     * @author qixin
     */
    public static void fitTitle(HSSFWorkbook book, HSSFSheet sheet, String fileName, int witch) {
        HSSFFont titleFont = book.createFont();
        titleFont.setFontHeightInPoints((short)24);
        titleFont.setFontName("SimHei");
    ...
    public static void fitInfos(HSSFWorkbook book, HSSFSheet sheet, List<LinkedHashMap<String, String>> treeMapList, Map<String,Object> treeMapData) {
        ExcelUtil.CREATE_ROW_NUMBER = 3;
        ...
        for(LinkedHashMap<String,String> treeMap : treeMapList){
            Row row = sheet.createRow(++ExcelUtil.CREATE_ROW_NUMBER);
     ...
     * @author qixin
     */
    public static void fitHeader(HSSFWorkbook book, HSSFSheet sheet, String[] headers) {
        ...
        Row header = sheet.createRow(++ExcelUtil.CREATE_ROW_NUMBER);
    ...
    static void fitContent(HSSFWorkbook book, HSSFSheet sheet, String[] withProperties, List<Map<String,Object>> dataLists) {
        ...
        for (int i = 1; i < dataLists.size()+1; i++) {
            Row dataRow = sheet.createRow(++ExcelUtil.CREATE_ROW_NUMBER);
    ...
    static void fitContent(HSSFWorkbook book, HSSFSheet sheet, String[] withProperties, List<Map<String,Object>> dataLists, String statistics) {
        ...
        dataCellStyle = CellStyleKit.rightCellStyle((HSSFCellStyle) dataCellStyle);
        dataCellStyle.setFont(dataFont);

        HSSFRow row = sheet.createRow(++ExcelUtil.CREATE_ROW_NUMBER);
    ...
    static void fitRemarks(HSSFSheet sheet, String[] remarks) {
        ...
        for(String str : remarks){
            row = sheet.createRow(++ExcelUtil.CREATE_ROW_NUMBER);
            title = row.createCell(0);
